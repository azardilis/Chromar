...
instance Monoid a => Monoid (Fluent a) where
  ...
  mappend f f' = Fluent { at = \t -> at f t `mappend` at f' t }
...
fconcat ts fs = let emptyF     = constant mempty
...
every t init acc = let times = scanl1 (+) (0:repeat t)
...
acc f ti init acc = let times = scanl1 (+) (repeat ti)
                        vals  = map (at f) times
...
accTable table init accF = Fluent { at = \t -> foldr
                                               (\(tt, v) acc -> accF v acc)
                                               init
                                               (tableUntilTime t table) }
  where tableUntilTime t = takeWhile (\(tt, v) -> tt <= t)
